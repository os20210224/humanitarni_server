...
public class Op_Uplata {
	...
	public static void uplata(Klijent_Info k) throws IOException {
		...
		while (!validna_kartica(k, kartica)) {
			Meni_Header.header(k, Podmeni.UPLATA);
		...
		// jasno iz naziva
		if (dodaj_uplatu(k, nova_uplata) && apdejtuj_stanje(k, nova_uplata)) {
	...
	public static void p_uplata(Klijent_Info k) throws IOException {
	...
	static boolean dodaj_uplatu(Klijent_Info k, Uplata uplata) {
		boolean status = true;
	...
	static String formatiraj_uplatu(Uplata uplata) {
		String uplata_string = String.format("| %20s ", uplata.ime) + String.format("| %20s ", uplata.prezime)
				...
				+ String.format("| %d.%d.%d. %d:%2d ", uplata.vreme.get(GregorianCalendar.DAY_OF_MONTH),
						uplata.vreme.get(GregorianCalendar.MONTH + 1), uplata.vreme.get(GregorianCalendar.YEAR),
						uplata.vreme.get(GregorianCalendar.HOUR_OF_DAY), uplata.vreme.get(GregorianCalendar.MINUTE))
				+ String.format("| %d |\n", uplata.iznos);
		return uplata_string;
	}

	static boolean apdejtuj_stanje(Klijent_Info k, Uplata uplata) {
		boolean status = true;
		ReadWriteLock lock = new ReentrantReadWriteLock();
		Lock upis_lock = lock.writeLock();
		try {
	...
	static boolean validna_kartica(Klijent_Info k, String kartica) {
		try {
			RandomAccessFile lista_kartica = new RandomAccessFile("baza.txt", "r");
			String kar;
			while ((kar = lista_kartica.readLine()) != null) {
				if (kar.contains(kartica)) {
					lista_kartica.close();
					return true;
				}
			}
